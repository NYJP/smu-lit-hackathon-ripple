...
what the /who picker renders before any session exists. Every authenticated
account has administrator write access.
"""
...
def create_user(
    ...
    conn.execute(
        ...
        (user_id, payload.display_name, "admin", _now()),
    )
    conn.commit()
...
def patch_user(
    ...
    # Role input is accepted for backwards-compatible clients but ignored.
