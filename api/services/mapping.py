...
from api.services import jobs, openai, retrieval
...
_DOCUMENT_DECISION_SCHEMA: dict[str, Any] = {
    "type": "object", "additionalProperties": False, "required": ["decisions"],
    "properties": {"decisions": {"type": "array", "items": {
        "type": "object", "additionalProperties": False,
        "required": ["requirement_id", "depends", "relationship_type", "confidence", "rationale", "evidence_span"],
        "properties": {
            "requirement_id": {"type": "string"}, "depends": {"type": "boolean"},
            "relationship_type": {"type": ["string", "null"], "enum": ["restates", "implements", "references", "defines", None]},
            "confidence": {"type": "number", "minimum": 0, "maximum": 1},
            "rationale": {"type": "string"}, "evidence_span": {"type": ["string", "null"]},
        },
...
def _candidate_chunks(conn: sqlite3.Connection, requirement: sqlite3.Row, document_ids: set[str] | None = None) -> list[sqlite3.Row]:
    ...
    return [
        ...
        and rows[identifier]["chunk_type"] != "heading"
        and len(" ".join(rows[identifier]["content"].split())) >= 60
        and (document_ids is None or rows[identifier]["document_id"] in document_ids)
    ]
...
def _adjudicate(
    ...
    *,
    job_conn: sqlite3.Connection | None = None,
    job_id: str | None = None,
) -> tuple[list[dict[str, Any]], openai.Usage]:
    if not candidates:
        return [], openai.Usage()
    supplied = "\n\n".join(
    ...
    completion = openai.structured_completion(
        ...
        job_conn=job_conn, job_id=job_id,
    )
    permitted = {chunk["id"] for chunk in candidates}
    decisions = [item for item in completion.value.get("decisions", []) if item.get("chunk_id") in permitted]
    if {item["chunk_id"] for item in decisions} != permitted or len(decisions) != len(permitted):
        raise openai.ExternalServiceError("Dependency output did not contain exactly one decision per chunk.")
    return decisions, completion.usage


def _adjudicate_requirements(
    chunk: sqlite3.Row,
    requirements: list[sqlite3.Row],
    *,
    job_conn: sqlite3.Connection | None = None,
    job_id: str | None = None,
) -> tuple[list[dict[str, Any]], openai.Usage]:
    """Adjudicate one document passage against up to eight requirements."""
    if not requirements:
        return [], openai.Usage()
    supplied = "\n\n".join(
        "\n".join(
            [
                f"Requirement {requirement['id']} ({requirement['public_ref']})",
                f"text: {requirement['requirement_text']}",
                f"subject: {requirement['subject']}",
                f"value: {requirement['value']}",
                f"condition: {requirement['condition']}",
                f"exception: {requirement['exception']}",
            ]
        )
    ...
    completion = openai.structured_completion(
        _SYSTEM,
        f"Document passage {chunk['id']} | section {chunk['section_path'] or 'unknown'}:\n"
        f"{chunk['content']}\n\nRegulatory requirements:\n{supplied}",
        _DOCUMENT_DECISION_SCHEMA,
        model=os.environ.get("RIPPLE_BULK_MODEL", "gpt-5-mini"),
        schema_name="document_dependency_decisions",
        job_conn=job_conn,
        job_id=job_id,
    )
    permitted = {requirement["id"] for requirement in requirements}
    decisions = [
        item for item in completion.value.get("decisions", [])
        if item.get("requirement_id") in permitted
    ]
    if {item["requirement_id"] for item in decisions} != permitted or len(decisions) != len(permitted):
        raise openai.ExternalServiceError(
            "Dependency output did not contain exactly one decision per requirement."
        )
    return decisions, completion.usage
...
def _write_dependency(conn: sqlite3.Connection, lineage_id: str, chunk: sqlite3.Row, decision: dict[str, Any], scan_id: str | None) -> int:
    ...
    existing = conn.execute(
        """SELECT id, status, relationship_type, evidence_span
           FROM dependencies WHERE lineage_id = ? AND document_chunk_id = ?""",
        (lineage_id, chunk["id"]),
    ).fetchone()
    if existing is None:
        conn.execute(
    ...
    if existing["status"] == "active":
        ...
        old_span = " ".join((existing["evidence_span"] or "").casefold().split())
        new_span = " ".join((evidence_span or "").casefold().split())
        materially_changed = existing["relationship_type"] != relationship or old_span != new_span
        if materially_changed:
            conn.execute(
                """UPDATE dependencies
                   SET status = 'active', relationship_type = ?, confidence = ?, rationale = ?,
                       evidence_span = ?, evidence_start = ?, evidence_end = ?, found_by_scan_id = ?
                   WHERE id = ?""",
                (relationship, float(decision["confidence"]), str(decision.get("rationale") or ""), evidence_span, evidence_start, evidence_end, scan_id, existing["id"]),
            )
            conn.execute(
                """INSERT INTO dependency_events
                   ...
                   VALUES (?, ?, 'reactivated', 'dismissed', 'active', ?, ?, ?, ?, ?, ?)""",
                (uuid.uuid4().hex, existing["id"], existing["relationship_type"], relationship,
                 existing["evidence_span"], evidence_span, scan_id, _now()),
            )
            return 1
    return 0
    ...
    conn.execute(
        """INSERT INTO mapping_passes
           (document_id, lineage_id, direction, dependency_found, candidates_seen,
            scan_id, mapped_at, basis_hash, materially_checked_at)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
...
def map_document(
    ...
    for chunk in chunks:
        ...
        for start in range(0, len(candidates), _BATCH_SIZE):
            ...
            try:
                decisions, call_usage = _adjudicate_requirements(
                    chunk, batch, job_conn=conn, job_id=job_id
                )
                usage = usage.add(call_usage)
                if job_id:
                    jobs.add_usage(conn, job_id, call_usage)
            except openai.ExternalServiceError:
                # Isolate one failed batch so unrelated candidates still map.
                failed_batches += 1
                decisions = []
                for requirement in batch:
                    try:
                        single, call_usage = _adjudicate_requirements(
                            chunk, [requirement], job_conn=conn, job_id=job_id
                        )
                        decisions.extend(single)
                        usage = usage.add(call_usage)
                        if job_id:
                            jobs.add_usage(conn, job_id, call_usage)
                    except openai.ExternalServiceError as exc:
                        failed_lineages.add(requirement["lineage_id"])
                        if job_id:
            ...
            requirements_by_id = {requirement["id"]: requirement for requirement in batch}
    ...
    for lineage in lineages:
        if lineage["id"] in failed_lineages:
            continue
        _write_pass(conn, document_id, lineage["id"], "document_first", found[lineage["id"]], candidates_seen[lineage["id"]], scan_id)
    conn.commit()
    return MappingResult(added, sum(candidates_seen.values()), usage, failed_batches)
...
def map_lineage(
    ...
    for start in range(0, len(chunks), _BATCH_SIZE):
        ...
        try:
            decisions, call_usage = _adjudicate(
                requirement, batch, job_conn=conn, job_id=job_id
            )
            usage = usage.add(call_usage)
            if job_id:
                jobs.add_usage(conn, job_id, call_usage)
            ...
            for chunk in batch:
                try:
                    single, call_usage = _adjudicate(
                        requirement, [chunk], job_conn=conn, job_id=job_id
                    )
                    decisions.extend(single)
                    usage = usage.add(call_usage)
                    if job_id:
                    ...
                    failed_documents.add(chunk["document_id"])
                    if job_id:
                        jobs.append_error_context(
                            conn,
                            job_id,
