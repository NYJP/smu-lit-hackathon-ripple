...
export function RequirementsPageContent() {
  ...
  return <PageFrame>
    <PageHeader eyebrow="Regulatory workspace" title="Requirements" description="Extracted obligations linked to the documents that depend on them." />
    {error ? <Surface><EmptyState icon={AlertCircle} title="Requirements unavailable" description={error} /></Surface>
      : items === null ? <Surface><LoadingSkeleton rows={6} /></Surface>
      : items.length === 0 ? <Surface><EmptyState icon={ListChecks} title="No requirements yet" description="Requirements will appear after a regulation has been processed." /></Surface>
      : <Surface className="overflow-x-auto"><Table><TableHeader><TableRow><TableHead>Reference</TableHead><TableHead>Requirement</TableHead><TableHead>Type</TableHead><TableHead>Source</TableHead><TableHead className="text-right">Dependencies</TableHead></TableRow></TableHeader><TableBody>{items.map(item => <TableRow key={item.lineage_id}><TableCell><Link href={`/requirements/${item.lineage_id}`}><Badge variant="outline" className="hover:bg-secondary">{item.public_ref}</Badge></Link></TableCell><TableCell className="max-w-xl font-medium"><Link className="hover:underline" href={`/requirements/${item.lineage_id}`}>{item.requirement_text}</Link>{item.value ? <span className="ml-2 text-muted-foreground">({item.value})</span> : null}</TableCell><TableCell className="capitalize">{item.requirement_type}</TableCell><TableCell className="text-muted-foreground">{item.source_section ?? "—"}</TableCell><TableCell className="text-right tabular-nums"><Link className="hover:underline" href={`/requirements/${item.lineage_id}`}>{item.dependency_count}</Link></TableCell></TableRow>)}</TableBody></Table></Surface>}
  </PageFrame>;
}
