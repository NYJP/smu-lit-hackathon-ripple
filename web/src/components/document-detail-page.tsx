...
type DocumentDetail = { document: { id: string; name: string; doc_type: string; mime_type: string; status: string; page_count: number | null; owner: { display_name: string } }; chunks: Chunk[] };
...
export function DocumentDetailPage() {
  ...
  const isPdf = data.document.mime_type === "application/pdf";
  return <div className="mx-auto w-full max-w-6xl px-5 py-8">
    ...
    <div className="mt-7 overflow-hidden rounded-lg border">{isPdf ? <iframe title={data.document.name} src={`${fileUrl}?inline=true`} className="h-[70vh] w-full bg-muted/20" /> : <div className="flex min-h-56 flex-col items-center justify-center gap-3 p-8 text-center"><ExternalLink className="size-8 text-muted-foreground" /><div><p className="font-medium">Preview is not available for this file type</p><p className="mt-1 text-sm text-muted-foreground">Open the original DOCX or TXT file using the download button.</p></div></div>}</div>
