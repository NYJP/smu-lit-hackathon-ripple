...
**Simulations are organisation-wide too.** Every signed-in account evaluates the same dependency set. Relevance may rank or explain results, but never hides or gates them.
...
Primary navigation, in this order: **Documents · Regulations · Requirements · Changes · Impacts · Graph · Simulations · Scans · Search · People · Settings**. A **Scan** control and the account menu sit in the header on every page. Access is deliberately all-admin in this local prototype.
...
| `/documents/[id]` | Document chunks in order with a 4px semantic severity rail, restrained severity tint, severity badge, and an underline on only the exact affected span. The server-provided `clause_label` is authoritative. Includes a severity count strip, **Only affected clauses**, previous/next affected-clause navigation, policy-source links, and approved wording rendered only as a `document_patches` overlay. |
...
| `/simulations/[id]` | Header and every hypothetical result carry the purple **Simulated** treatment. The editable current/proposed comparison is followed by persisted named job progress, then a skippable ripple (origin pulse → travelling dependency dots → target severity rings and one pulse → settled result). Reduced motion applies final rings immediately. The summary reports documents examined, affected, severity counts, likely unaffected, and confidence. **Discard** removes only simulation artifacts and returns to `/simulations`. |
...
- **Simulated content is never mistakable for real.** Any change, impact, graph state, or recommendation whose `source = 'simulation'` carries the purple "Simulated" chip in every list, card, inspector, and detail header. Purple is reserved for hypothetical state and is distinct from every severity/status token.
...
**Implemented renderer:** `react-force-graph-2d` (canvas + d3-force), loaded client-side with SSR disabled. Live force physics, a 250-node API/render cap, and a complete keyboard-accessible table alternative keep the view responsive while preserving every relationship in a non-canvas form. Confidence tightens link springs; drag, pan, zoom, pin, re-centre, local/global modes, and URL-restored selection and filters are supported.
...
- **Single configured gateway.** The official `openai` Python SDK uses `OPENAI_BASE_URL=https://openrouter.ai/api/v1`; the current key is an OpenRouter key. Embeddings and generation use the configured `RIPPLE_*_MODEL` values through that gateway. Do not silently replace it with `api.openai.com`.
